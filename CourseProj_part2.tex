...
\textasciitilde{}6.4 (smallest diff observed). Therefore, we can say
with some confidence that even with the smallest group difference, the
differences between dose levels is significantly different. From this,
we may infer that dose level does have an effect on tooth growth.
However, the previous t-test showed that the type of supplement does not
have a net effect.
...
\item
  We have 95\% power to correctly reject the null hypothesis at the
  smallest group difference observed (\(\Delta = 6.4\))
\end{enumerate}
