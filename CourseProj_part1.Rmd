...
p3 <- ggplot(data2, aes(x=Value)) + geom_histogram(aes(y = ..density..), binwidth=0.5, fill="white", color="blue") + geom_vline(xintercept = mu, color="red") + facet_grid(.~variable) + stat_function(fun = dnorm, args = list(mean=1/lambda, sd = s), n = reps, color="red", lty=5, lwd=1.5) +  theme_bw() + labs(title = "A population of random exponentials\nvs. Sample means of random exponentials") + theme(plot.title = element_text(size = 10), axis.title.x=element_text(size=10), axis.title.y=element_text(size=10)) + coord_cartesian(xlim=c(0,10)) 
