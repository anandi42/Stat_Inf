...
t1 <- data.frame( Group=c(levels(datasupp$supp), levels(datadose$dose)), Means=c(datasupp$mean[1:2], datadose$mean[1:3]), Variances=c(datasupp$var[1:2], datadose$mean[1:3]), Counts = c(datasupp$count[1:2], datadose$count[1:3]))
print(xtable(t1, digits = 3), comment=FALSE,include.rownames=FALSE)
```  
...
sd1 <- sqrt(vars1)
rbind(meandiff, sd1)
power.t.test(n=10, type = "two.sample", alternative = "two.sided", 
             sig.level = 0.01, sd=4.5, delta=6.4)$power
power.t.test(power = 0.95, type = "two.sample", alternative = "two.sided", 
             sig.level = 0.01, sd=4.5, delta=6.4)$n
```  
  At an $n$ of 10, and $\alpha=0.01$ we have 62% power to correctly reject a null hypothesis assuming a true mean difference of ~6.4 (smallest diff observed). Based on a power calculation at power of 95% and $\alpha = 0.01$, we may need up to 20 gerbils per group to correctly reject the null hypothesis when $\Delta = 6.4$. 
...
#Conclusions
  ...
  2. We infer from this data that while the *type* of Vitamin C supplement *does not* significantly affect tooth growth, higher *doses* of Vitamin C *may have an effect on tooth growth*.
