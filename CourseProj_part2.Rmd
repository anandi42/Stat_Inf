...
table1 <- data.frame( Group=c(levels(datasupp$supp), levels(datadose$dose)), Means=c(datasupp$mean[1:2], datadose$mean[1:3]), Variances=c(datasupp$var[1:2], datadose$mean[1:3]), Counts = c(datasupp$count[1:2], datadose$count[1:3]))
...
```  
  At an $n$ of 20/group, and $\alpha=0.01$ we have 95% power to correctly reject a null hypothesis assuming a true mean difference of ~6.4 (smallest diff observed). Therefore, we can say with some confidence that even with the smallest group difference, the differences between dose levels is significantly different. From this, we may infer that dose level does have an effect on tooth growth. However, the previous t-test showed that the type of supplement does not have a net effect. 
...
#Conclusions
  ...
  3. We have 95% power to correctly reject the null hypothesis at the smallest group difference observed ($\Delta = 6.4$)
